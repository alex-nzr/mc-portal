...
namespace Cbit\Mc\Subscription\Internals\Control;
...
use Cbit\Mc\Subscription\Controller;
use Cbit\Mc\Subscription\Entity;
use Cbit\Mc\Subscription\Filter\FilterFactory;
use Cbit\Mc\Subscription\Service\Container;
use Cbit\Mc\Subscription\Service\Integration\Intranet\CustomSectionProvider;
use Cbit\Mc\Subscription\Service\Router;
use Exception;

/**
 * Class ServiceManager
 * @package Cbit\Mc\Subscription\Internals\Control
 */
class ServiceManager
{
    ...
    /**
     * @return \Cbit\Mc\Subscription\Internals\Control\ServiceManager
     */
    ...
    {
        $dependencies = [
            'cbit.mc.core', Constants::RI_MODULE_ID,
        ];
    ...
    {
        if (Container::getInstance()->getRouter()->isDetailPage() || Container::getInstance()->getRouter()->isListPage())
        {
    ...
    {
        Extension::load([
            'cbit.mc.subscription.ui-detail'
        ]);
    }
